...
class CmsRoleBasedAccessMethods < CmsBasePage
  ...
  def nav_to_agents_screen
    ...
    expect(page).to have_current_path(%r{#{current_user_state.base_url}/management/agents#current-accounts}, url: true, wait: 10)
    expect(cms_man_agents_list_comps.text_page_heading.text).to include("Agents")
  end

  def set_single_role_based_permission(agent, permission)
    # Find and edit our current target user
    cms_man_agents_list_comps.link_current_staff.click
    if agent == "rba_test_user_login"
      cms_man_agents_list_comps.link_edit_by_name("Auto 3 Test User 3").click
      expect(page).to have_current_path(%r{/edit}, url: true, wait: 10)
      expect(cms_man_agents_edit_agent_comps.text_page_heading.text).to include("Edit Auto 3 Test User 3")
    end
    ...
    cms_man_agents_edit_agent_comps.button_save.click
    ...
    validate_value_contains(permission, cms_man_agents_list_comps.text_see_roles_by_name("Auto 3 Test User 3").text)
  end
...
private
  ...
  def reset_all_checkboxes
    rba_roles = cms_man_agents_edit_agent_comps
  ...
  def select_permission(permission)
    case permission
    when "Global Administrator"
      cms_man_agents_edit_agent_comps.label_global_administrator.set(true)

    when "Procurement Operations Admin"
      cms_man_agents_edit_agent_comps.label_procurement_operations_admin.set(true)

    when "Procurement Operations Staff Member"
      cms_man_agents_edit_agent_comps.label_procurement_operations_staff_member.set(true)

    when "Engagement and Outreach Admin"
      cms_man_agents_edit_agent_comps.label_engagement_and_outreach_admin.set(true)

    when "Engagement and Outreach Staff Member"
      cms_man_agents_edit_agent_comps.label_engagement_and_outreach_staff_member.set(true)

    when "Digital Team Staff Member"
      cms_man_agents_edit_agent_comps.label_digital_team_staff_member.set(true)

    when "Data Analyst"
      cms_man_agents_edit_agent_comps.label_data_analyst.set(true)

    when "Framework Evaluator Admin"
      cms_man_agents_edit_agent_comps.label_framework_evaluator_admin.set(true)

    when "Framework Evaluator"
      cms_man_agents_edit_agent_comps.label_framework_evaluator.set(true)

    when "CEC Staff Member"
      cms_man_agents_edit_agent_comps.label_cec_staff_member.set(true)

    when "CEC Admin"
      cms_man_agents_edit_agent_comps.label_cec_admin.set(true)
    else
