...
class EnergyAreYouAuthorisedPageMethods < EnergyBasePage
  ...
  def continue_to_what_are_switching_page
    ...
    log_debug("full path after settle: #{path}")

    if path.include?("/authorisation")
      log_info("On /authorisation — continue as normal")
      ...
      log_info("On /task-list — handle reset flow")
      ...
      log_info("Unknown path '#{path}', skipping optional logic")
    end
...
private
  ...
  def wait_until_authorisation_or_tasklist_if_on_school_picker(timeout: 7)
    ...
    log_info("On /which-school-buying-for — waiting (up to #{timeout}s) for /authorisation or /task-list…")
    begin
      Capybara.using_wait_time(timeout) do
        expect(page).to have_current_path(%r{/(authorisation|task-list)}, url: true)
      end
    rescue RSpec::Expectations::ExpectationNotMetError
      log_warn("Still not on /authorisation or /task-list after #{timeout}s (current: #{current_url})")
    end
